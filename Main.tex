...
concentrating on a single particular problem, however theoretically they are 
interdependent.
Core intuition of this paper is that the learned artifacts during termination and nontermination
analysis can be used to guide the analysis.
We present a new approach to termination analysis based on the interpolation-based 
construction of well-founded transition invariants. By leveraging Craig interpolation, 
our approach guides transition invariant generation with respect to the conditions 
relevant to termination, enabling property-directed synthesis. 
Our technique extends the safety-based nontermination analysis, allowing it to prove both termination and 
nontermination within a unified framework.
We implemented our approach and evaluated it on benchmarks from the 
 Termination Competition (TermComp). Our tool demonstrates 
competitive performance against state-of-the-art termination provers, solving 
a significant number of instances not solved by any existing tool.
\end{abstract}
