...
\begin{example}\label{Ex:Motivation}
    ...
    $X = \{v, u\}$, where $v, u$ are integer variables, $Init(X)   \triangleq u < 10 \land v > 0 $, and
    $Tr(X, X') \triangleq ((v' = v - 1 \land u' = u ) \lor (v' = v + 1 \land u' = u + 1 \land u < 10)) \land v > 0$.


    Intuitively, at each step the system either decrements $v$ (and keeps $u$ unchanged), or increments both $v$ and $u$ 
    provided $u < 10$. 
    Once $u = 10$, only the decrementing branch is available, and $v$ decreases monotonically to zero. 
    ...
    grows until at most $u = 10$, after which $v$ decreases monotonically. 
