...
\begin{example}\label{Ex:Tn}
    ...
    Consider $TS = \tuple{Init, Tr, X}$ with $Init \triangleq  v > 0$, $Tr(X,X') \triangleq  ((v' = v - 1 \lor v' = v - 2) \land v > 0) \lor (v < -10 \land v' = v - 1)$, $X \triangleq  \{v\}$.
    In this TS, sink states can be encoded as $Sink(X) \triangleq v \leq 0 \land v \geq -10$.
    For $n = 2$ quantifier elimination would yield $NT^2(X) = v > 2$.
    States with $v \in \{3,4\}$ can reach $Sink$ in two transitions, but not necessarily.
...
since $\lnot C(X) \triangleq v < 0$ is the sink states, and for all other states $TrInv$ is a valid
transition invariant.
...
\begin{example}
    ...
    \trInvSynthesis produces a transition invariant candidate $TInv(X,X') \triangleq v' < v \land v' \geq 0$ and the formula $C(X) = v > 0$.
    The safety check for $\tuple{Init, Tr, \lnot (C(X) \lor Sink(X)), v}$ returns SAFE, and algorithm concludes that the transition system is terminating.
\end{example}
