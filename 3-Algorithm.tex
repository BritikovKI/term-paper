...
A well-founded candidate transition invariant is then constructed based on the overapproximation of the terminating trace.
...
(1) computing $NT^n$, the formula encoding states, from which termination is not necessarily reached within $n$ transitions.
...
\begin{example}
    ...
    The Algorithm~\ref{Alg:SNA_Term} would approach this transition system in a similar manner,
    detecting a trace $Tr^1(X^{(0)}, X^{(1)})$ reaching $Sink$ states.
    ...
    In Section~\ref{Sec:Extension}, we propose an extension which would allow to concentrate on the state space
    ...
    \While{$\top$} {
        ...
        \eIf{$\tuple{res, \phi} = \tuple{UNSAFE, Tr^{n}(X^{(0)}, \dots, X^{(n)})}$} {
            ...
            }
            {
                ...
                $S_m \gets QE(\exists X^{(0)}, \dots, X^{(m-1)}, Init(X^{(0)}) \land \phi'(X^{(0)}, \dots, X^{(m)}) \land \lnot C(X^{(m)}))$ \label{SNA_Refined:Sm}\\
...
When $\lnot C$ is reachable, the extended algorithm performs two actions: (3) it pushes the $\blockTerm$-refined problem 
(line~\ref{SNA_Refined:PushNewProblem}); (2) it constructs formula, encoding states reachable within transition system,
...
Otherwise, if $\lnot C$ is not reachable, $TrInv$ proves termination of the currently analyzed TS (which may be a refined TS 
...
continue rather than returning UNKNOWN. 
...
\begin{proof}
    ...
    follows directly from the Lemma~\ref{Lem:Term}. This is the case, since the original safety problem is
    preserved in the stack, so if $Prs = \empty$, the original safety problem is necessarily terminating.
