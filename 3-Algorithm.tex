...
transitions. It is defined by the following property: $\forall X^{(0)}. NT^n(X^{(0)}) \leftrightarrow $
...
This allows us to compute the \emph{Craig interpolant}, which over-approximates the trace $Tr^{n}$.
...
The disjuncts for which a ranking function exists are preserved, the rest are discarded.
...
\begin{example}
    ...
    The safety check for $\tuple{Init, Tr, \lnot C(X), x}$ returns \SAFE, and the algorithm concludes that the transition system is terminating.
    ...
    In Section~\ref{Sec:Extension}, we propose an extension that concentrates on the state space
...
This subsection proves the soundness of the introduced approach. We first prove that when the algorithm returns \TERM, the transition system is nonterminating,
...
\end{lemma}
\begin{proof}
    The proof of this lemma follows directly from Lemma~\ref{Lem:NONTERM}, as the nontermination analysis procedure 
...
\begin{figure}[t]
    ...
    \caption{Optimized Interpolation-based Termination Analysis (New components are highlighted in dark orange and dark blue colors).}
    \label{fig:SNA++}
\end{figure}
...
Additionally, the algorithm was extended with two procedures.
...
This means that the produced $Inv$ contains sink states itself, due to the updated $Tr$. 
