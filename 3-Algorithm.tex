...
$$\exists \dots, X^{(n)}. NT^{n}(X^{(0)}) \land Tr^{n}(X^{(0)}, \dots, X^{(n)}) \land \lnot Sink(X^{(n)})$$
...
under-approximating DNF formula on demand using model-based 
projection. 
Concretely, the disjuncts are constructed via syntactic processing of the formula,
unwrapping the innermost disjunctions and conjunctions.
The result is a formula:
$$Cs(X,X') \gets \bigvee_i=0^m d_i$$
where each $d_i$ is a conjunction, and $Cs(X,X') \implies itp(x,x')$.
...
in the worst case, $Cs$ fails to be a transition invariant, but 
this is equally possible even with a complete DNF conversion.
...
This section describes an extension of the algorithm that increases efficiency through better interaction between termination and nontermination
analysis.
Additionally, we present further extensions to the original SNA algorithm that improve overall performance. 
The overall high-level flow of the extended algorithm can be seen in Figure~\ref{fig:SNA++}.
...
If $\lnot C(X)$ is reachable, it means that exist states for which the transition invariant is not valid, and which potentially can lead to non-termination.
...
system, thereby covering previously uncovered states.
...
the TS, thereby efficiently expanding the transition invariant.
...
The postprocessing phase of transition invariant construction is the main change of the Algorithm~\ref{Alg:SNA_Term}.
When $\lnot C$ is reachable, our algorithm performs two key actions: (1) it pushes the $\blockTerm$-refined problem (as in the original algorithm), and (2) it additionally pushes a new subproblem where  
...
Otherwise, if $\lnot C$ is not reachable, then $TInv$ proves termination of the currently analyzed TS (which may be a refined TS 
...
$$Init(X) \land Tr(X,X')$$
If this formula is unsatisfiable, then no transition can be taken from any initial state (or all transitions are blocked, 
...
These states are themselves sink states under the refined $Tr'$, and can be computed as:
...
\begin{proof}
    ...
    ased on Lemma~\ref{Lem:NONTERM}, if $NONTERM$ is returned, then there exists a recurrent set in the transition system $\tuple{Init', Tr', X}$, which 
