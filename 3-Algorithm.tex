...
\paragraph{Step 1} \textit{Computing $T^n$ (line~\ref{SNA_TrInv:T}).} 
...
\paragraph{Step 2} \textit{Interpolation (line ~\ref{SNA_TrInv:Itp}).}
...
\paragraph{Step 3} \textit{DNF underapproximation (line ~\ref{SNA_TrInv:toDNF}).}
...
\paragraph{Step 4} \textit{Filtering Well-Founded Disjuncts (line ~\ref{SNA_TrInv:TINV}).}
...
\paragraph{Step 5} \textit{Synthesis of the states for which transition invariant is valid (line ~\ref{SNA_TrInv:NC}).}
...
\begin{algorithm}[t!]
    ...
    \Data{$TInv$ - formula that encodes transition invariant, $Prs$ - list of Safety Problems to be
