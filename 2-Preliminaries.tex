...
A \emph{transition system} (TS) is defined as $TS = \tuple{ Init, Tr, X }$, where $Init$ is a formula 
...
A \emph{safety problem} is defined as  $P = \tuple{ Init, Tr, Bad, X }$, where $Bad$ is a formula representing 
...
\textit{Termination:} A TS is called \textbf{terminating} if for every initial state it is possible to take only finite number of transitions.
...
Using quantifier elimination, Algorithm~\ref{Alg:SNABlock} computes $S_i$ and $S_{i+1}$, 
...
Otherwise, \blockTerm blocks the subset of initial states that deterministically leads to sink (line~\ref{SNA:BLOCKINIT}).
