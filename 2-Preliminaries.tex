...
\begin{definition}[Sink states]
    ...
    such that $\forall X. Sink(X) \leftrightarrow \lnot \exists X'. Tr(X,X')$
\end{definition}
...
elimination at the lines~\ref{SNA:Si1},~\ref{SNA:Si}.
Then, at line~\ref{SNA:BLOCK} checks if $S_i(X) \land Tr(X,X') \land \lnot S_{i+1}(X')$ is satisfiable, sending a query to the SMT solver.
This query is needed to check if it is possible to reach some state does not necessarily leads to the sink states.
...
\begin{example}\label{Ex:SNA}
    ...
    Let us examin the execution of Algorithm~\ref{Alg:SNA} on this example.
    First, using \blockTerm it blocks initial states that lead to instant termination:
    $Init(X) \;\gets\; x > 0$.
