...
\textit{Termination:} A $\TS$ is called \textbf{terminating} if for every initial state it is possible to take only a finite number of transitions.
...
A relation $W(X,X')$ can be proven~\cite{DBLP:conf/vmcai/PodelskiR04} to be well-founded by synthesizing a ranking function $f(X)$, such that
 $\forall X,X'. W(X,X') \rightarrow f(X) > f(X')$.

A transition invariant can be used to prove termination~\cite{DBLP:conf/lics/PodelskiR04}.
...
\textit{Craig Interpolation:} Given two formulas $A$ and $B$ where $A \land B$ is unsatisfiable, a Craig interpolant~\cite{Craig_1957} 
for $A$ and $B$ is a formula $I$, where: (1) $A \rightarrow I$; 
(2) $I \land B$ is unsatisfiable; and (3) $I$ only contains the symbols, 
common to both $A$ and $B$.
The Craig interpolant serves as an overapproximation technique in our approach.
We use $\textsc{Itp}(A, B)$ to denote the computation of the Craig interpolant.
