...
\begin{definition}[Disjunctively well-founded relation]
    A relation $W(X,X') = \bigcup_{i = 0}^{n} R_i(X,X')$ is called disjunctively well-founded if every relation $R_i(X,X')$ is well-founded. 
\end{definition}
...
Craig interpolant is used in our approach as an abstraction technique, that can overapproximate formulae.
