...
\begin{example}\label{Ex:SNA}
    ...
    This transition system is nonterminating, since $x$ can decrease 
    indefinitely through negative values.

    The execution of Algorithm~\ref{Alg:SNA} updates initial states
    and transition in two iterations of the algorithm, using \blockTerm\: 
    ...
    Algorithm~\ref{Alg:SNA}, would proceed with the execution in a similar manner.
    First, using \blockTerm it would block initial states that lead to instant termination:
    $Init(X) \;\gets\; x > 0$.
    ...
    termination.
    Particularly in this example, the formula $TrInv(X, X') \triangleq x' < x \land x > 0$ 
    is well-founded transition invariant which is sufficient to prove termination of $TS$.
