...
A transition invariant~\cite{DBLP:conf/lics/PodelskiR04} can be used to prove termination.
...
quantifier elimination, $Sink(X) = QE(\exists X'. \lnot Tr(X,X'))$.
...
When $P$ is safe, algorithm checks if inductive invariant $Inv(X)$ is a recurrent set (line~\ref{SNA:SAFE}).
...
Otherwise, algorithm returns UNKNOWN.
...
\begin{example}\label{Ex:SNA}
    ...
    $Inv(X)$ is a recurrent set, and algorithm returns \textsc{NONTERM}.
